...
class MapViewport(
    ...
    init {
        ...
        val topHalfSurfaceDistance = sin(halfFov) * cameraToCenterDistance / sin(
            (pi - groundAngle - halfFov).clamp(
                0.01,
                pi - 0.01
            )
        )
        val furthestDistance =
            cos(halfPi - camera.pitch.radians) * topHalfSurfaceDistance + cameraToCenterDistance
    ...
    fun project(latitude: Scalar, longitude: Scalar, altitude: Scalar = 0.0): Vector2 {
        ...
        val worldPosition = Vector3(
            flatWorldPosition.x,
            flatWorldPosition.y,
            MercatorProjection.zFromAltitude(altitude, latitude) * MercatorProjection.worldSize(
                camera.zoom.zoomToScale
            )
        )
        val decartViewportPosition = (Vector4(worldPosition, 1.0) * pixelMatrix).toVector3()
    ...
    fun getIntersectedPolygon(polygons: List<ExtrudedPolygon>, point: Vector2): ExtrudedPolygon? {
        ...
        val worldLineP1 =
            Vector3(inverseViewMatrix.m41, inverseViewMatrix.m42, inverseViewMatrix.m43)
    ...
    private fun worldTransform(
        ...
        val flatWorldPosition: Vector2 =
            MercatorProjection.project(latitude, longitude, camera.zoom)
        return Vector3(
            flatWorldPosition.x,
            flatWorldPosition.y,
            MercatorProjection.zFromAltitude(altitude, latitude) * MercatorProjection.worldSize(
